...
const login = async (req,res) =>{
    ...
    const token = jwt.sign({id:user.id,name:user.name}, process.env.JWT_SECRET, {expiresIn:"2d"})
...
const register = async (req,res) =>{
    ...
    if(!name || !email || !password) return errorResponse(res, "silahkan isi semua field")
    ...
    if(emailDB) return errorResponse(res, "email sudah terdaftar silahkan pakai email lain")
